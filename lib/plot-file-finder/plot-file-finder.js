...
class PlotFileFinder {
  constructor() {
    ...
    this.windowsDrives = alphabet.map(char => `${char}:\\`);
    this.linuxMountDirectories = [
      '/mnt',
      '/media',
    ];
    this.appleMountDirectories = [
      '/Volumes',
    ];
  ...
  async findPlots() {
    ...
    const rootDirs = [].concat(this.windowsDrives, this.linuxMountDirectories, this.appleMountDirectories).filter(path => existsSync(path));
  ...
  get plotDirectories() {
    return [...new Set(this.plots.map(plot => plot.directoryPath))].sort();
  }
