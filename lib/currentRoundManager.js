...
class CurrentRoundManager {
  ...
  removeOldRoundsFromUpstream(upstream) {
    this.roundQueue = this.roundQueue.filter(currRound => currRound.upstream !== upstream);
  }

  removeCurrentRoundIfSameUpstream(upstream) {
    if (this.currentRound.upstream !== upstream) {
      return;
    }
  ...
  addNewRound(upstream, miningInfo) {
    ...
    this.removeOldRoundsFromUpstream(upstream);
    ...
    this.removeCurrentRoundIfSameUpstream(upstream);
