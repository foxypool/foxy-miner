...
class ProfitabilityService {
  ...
  async updateRates() {
    try {
      const rates = await this.coinGecko.getRates(['bitcoin-hd', 'burst']);
    ...
    try {
      const tickerLHD = await this.bitmartApi.getTicker('LHD_BHD');
      this.rates.lhd = parseFloat(tickerLHD.current_price) * (this.rates.bhd || 0);
    } catch (err) {
      this.rates.lhd = 0;
    }
  }
