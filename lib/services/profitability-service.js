...
class ProfitabilityService {
  ...
  getBlockReward(miningInfo, coin) {
    switch(coin) {
      case 'bhd': return 14.25;
      case 'burst':
        const month = Math.floor(miningInfo.height / 10800);
        return Math.floor(10000 * Math.pow(95, month) / Math.pow(100, month));
      case 'boom': return 190;
      case 'disc': return 17.5; // Basically impossible to reach full rewards consistently
      case 'lhd': return 92;
    }
  ...
  async updateRates() {
    try {
      ...
      this.rates.bhd = (rates['bitcoin-hd'].usd < 8 ? 12.5 : rates['bitcoin-hd'].usd);
