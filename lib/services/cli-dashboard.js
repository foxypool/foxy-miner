  ...
  buildTable() {
    ...
    upstreams.forEach(upstream => {
      ...
      if (upstream.miners.length === 0) {
        ...
        roundProgressText = `${upstream.roundProgress.toFixed(roundProgressRounding)} %\n`;
        roundProgressText += upstream.miners.map(minerStats => `${outputUtil.getString(`Miner #${minerStats.proxyIndex}`, minerStats.color)} | Scanning with ${minerStats.scanSpeed}, ${minerStats.remainingTime} (${minerStats.progress} %)`).join('\n');
      }
