  ...
  buildTable() {
    ...
    upstreams.forEach(upstream => {
      ...
      if (upstream.miners.length === 0) {
        ...
        roundProgressText = `${upstream.roundProgress.toFixed(roundProgressRounding)} %\n`;
        roundProgressText += upstream.miners.map(minerStats => `Miner #${minerStats.proxyIndex} | Scanning with ${minerStats.scanSpeed}, ${minerStats.remainingTime} (${minerStats.progress} %)`).join('\n');
      }
