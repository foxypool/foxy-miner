...
class Config {
  ...
  static get poolDescription() {
    return {
      ...
      '50-50': '50% of the block reward are distributed to historical shares of miners, 50% to the block winner.',
  ...
  async setupWizard() {
    ...
    await prompts({
      type: 'text',
      name: 'exit',
      message: chalk.green(`The setup wizard completed successfully and the config has been written! Head over to your ${minerType} config and change the url to ${chalk.white('http://' + listenAddr)}. Press enter to exit the program.`),
    });
    process.exit(0);
  }
