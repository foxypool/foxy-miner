...
class Config {
  ...
  patchConfig() {
    let updated = false;
    if (this._config.minerConfig && this._config.minerConfig.plotDirs && typeof this._config.minerConfig.plotDirs === 'string') {
      this._config.minerConfig.plotDirs = [this._config.minerConfig.plotDirs];
      updated = true;
    }
    if (this._config.listenAddr.indexOf('http') === 0) {
      this._config.listenAddr = '127.0.0.1:5000';
      updated = true;
    }
    if (updated) {
      this.save();
