...
class ScavengerConfigGenerator {
  ...
  generate({ minerConfig, minerIndex = null }) {
    ...
    const generatedConfig = {
      ...
      gpu_worker_task_count: this.getFromMinerConfig(minerConfig, { key: 'gpuWorkers', defaultValue: isCpuOnly ? 0 : 4 }),
      ...
      gpu_async: this.getFromMinerConfig(minerConfig, { key: 'useGpuAsyncCompute', defaultValue: false }),
  ...
  updateConfig({ configYaml, minerConfig, minerIndex = null }) {
    ...
    if (plotDirs !== null && config.hdd_reader_thread_count !== plotDirs.length) {
      config.hdd_reader_thread_count = plotDirs.length;
    }
