...
class Proxy {
  ...
  constructor(upstreamConfigs, proxyIndex, showProxyIndex = false) {
    this.proxyIndex = proxyIndex;
    this.showProxyIndex = showProxyIndex;
    this.upstreamConfigs = upstreamConfigs;
    this.currentRoundManager = new CurrentRoundManager(proxyIndex); // Default Round Manager
    ...
    if (assumeScannedAfter) {
      eventBus.subscribe('miner/new-round', async (miningInfo) => {
        ...
        eventBus.publish('miner/round-finished', proxyIndex, miningInfo.height);
      });
    }
  }
  ...
  async init() {
    this.upstreams = await Promise.all(this.upstreamConfigs.map(async upstreamConfig => {
      ...
      const upstream = new upstreamClass(upstreamConfig, this.showProxyIndex ? this.proxyIndex : false);
