    ...
    this.upstreams = await Promise.all(this.upstreamConfigs.map(async upstreamConfig => {
      ...
      upstream.on('new-round', (miningInfo) => {
        if (config.useProfitability) {
          ...
          eventBus.publish('log/debug', `Profitability-Service | ${upstream.upstreamName} | Set weight ${upstream.prio}`);
        }
        if (this.upstreams && config.maxNumberOfChains) {
          const prio = (upstream.prio || upstream.upstreamConfig.prio || upstream.upstreamConfig.weight) || 10;
          const upstreamsWithPrio = this.upstreams
            .map(upstream => [upstream, (upstream.prio || upstream.upstreamConfig.prio || upstream.upstreamConfig.weight) || 10])
            .sort((a, b) => b[1] - a[1])
            .slice(0, config.maxNumberOfChains)
            .reverse();
          if (!upstreamsWithPrio.some(upstreamWithPrio => upstreamWithPrio[0] === upstream) && prio <= upstreamsWithPrio[0][1]) {
        ...
        eventBus.publish('miner/new-round');
