...
const { getPlatformInfo, OpenclUnavailableError } = require('opencl-info');
    ...
    if (!isCpuOnly) {
      ...
      try {
        ...
        if (!(err instanceof OpenclUnavailableError)) {
          ora().fail(`An error occurred, not selecting any GPU! Error: ${err.message}`);
        }
      }
      ...
      const gpus = devices.filter(device => device.type === 'GPU');
      if (gpus.length === 1) {
        selectedGpu = gpus[0];
