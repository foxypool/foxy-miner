...
class FirstRunWizard {
  ...
  async run() {
    ...
    if (!isCpuOnly) {
      ...
      const deviceChoices = flatMap(platformInfo, (platform => platform.devices.map(device => ({
        title: `${device.vendor} - ${device.name}`,
        value: { platformId: platform.id, deviceId: device.id, cores: device.cores },
      }))));
      if (deviceChoices.length === 1) {
        gpuPlatform = deviceChoices[0].value.platformId;
        gpuDevice = deviceChoices[0].value.deviceId;
        gpuWorkers = Math.min(gpuWorkers, deviceChoices[0].value.cores);
        ora().succeed(`Using ${deviceChoices[0].title}`);
      } else if (deviceChoices.length > 1) {
        const { selectedGpu } = await prompts([{
          type: 'select',
          name: 'selectedGpu',
        ...
        gpuPlatform = selectedGpu.platformId;
        gpuDevice = selectedGpu.deviceId;
        gpuWorkers = Math.min(gpuWorkers, selectedGpu.cores);
      }
    }
