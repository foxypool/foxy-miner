...
const { getPlatformInfo } = require('gpu-detection');
...
class FirstRunWizard {
  ...
  async run() {
    ...
    if (!isCpuOnly) {
      ...
      if (devices.length === 1) {
        gpuPlatform = devices[0].value.platformId;
        gpuDevice = devices[0].value.deviceId;
        gpuWorkers = Math.min(gpuWorkers, devices[0].value.cores);
        ora().succeed(`Using ${devices[0].title}`);
