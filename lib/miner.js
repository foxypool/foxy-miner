...
class Miner {
  ...
  modifyDeadline(data) {
    ...
    return data.replace(this.deadlineRegex, (match, deadline) => {
      ...
      return `${config.compact ? '' : 'deadline='}${outputUtil.getString(Miner.getFormattedDeadline(deadline), Miner.getDeadlineColor(deadline))}${(config.compact && data.length < 82) ? ' '.repeat(86 - data.length) : ''}`;
    });
  }
