...
class Miner {
  constructor(binPath, configPath = null) {
    ...
    this.timestampRegex = /(([0-9]+:[0-9]+:[0-9]+) \[(.+)\]) +/;
    this.version = null;
  ...
  async start() {
    ...
    this.minerRef.stdout.on('data', (data) => {
      ...
      const lines = flatten(data.split('\n').map(line => line.trim().split('\r')));

      lines.forEach(line => {
        let logLevel = 'info';
        line = line.replace(this.timestampRegex, (match, all, time, level) => {
        ...
        if (line.indexOf(' % ') !== -1) {
          ...
          const [, scanSpeed] = line.match(this.scanSpeedRegex);
          this.scanSpeed = scanSpeed;
          const matches = line.match(this.remainingTimeRegex);
          this.remainingTime = matches ? matches[2] : null;
  ...
  set proxy(proxy) {
    this._proxy = proxy;
    this.fullMinerName = proxy.showProxyIndex ? `Miner #${proxy.proxyIndex} | ${this.constructor.name}` : this.constructor.name;
  }
