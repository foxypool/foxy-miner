...
class Miner {
  ...
  modifyDeadline(data) {
    ...
    return data.replace(this.deadlineRegex, (match, deadline) => {
      ...
      return `${config.hideDeadlinePrefix ? '' : 'deadline='}${outputUtil.getString(Miner.getFormattedDeadline(deadline), Miner.getDeadlineColor(deadline))}${config.hideDeadlinePrefix ? ' '.repeat(16) : ''}`;
    });
  }
  ...
  async start() {
    ...
    this.minerRef.stdout.on('data', (data) => {
      ...
      if (config.hideNonces && data.indexOf('nonce=') !== -1) {
