...
class Miner {
  ...
  modifyDeadline(data, isError = false) {
    ...
    return data.replace(this.deadlineRegex, (match, deadline) => {
      ...
      return `${config.compact ? '' : 'deadline='}${outputUtil.getString(Miner.getFormattedDeadline(deadline), Miner.getDeadlineColor(deadline))}${(config.compact && !isError) ? ' '.repeat(16) : ''}`;
    });
  }
  ...
  async start() {
    ...
    this.minerRef.stderr.on('data', (data) => {
      ...
      data = this.modifyDeadline(data, true);
